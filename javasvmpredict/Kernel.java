...
/**
     * Calculates the square norm of vectors
     *
     * @param x the first vector
     
     * @return The sum of the squares of vectors
     */
    public static double squarenorm(FeatureNode[] x) {
        double sum = 0;
        int i ;
     ...
     */
    public static double kGaussian(FeatureNode[] x, FeatureNode[] z, double sigma, double b) {
        //return Math.exp(euclidean_dist2(x, z) * (-0.5 / (sigma * sigma)));
       //return Math.exp(-(sigma)*euclidean_dist2(x, z));
        return Math.exp(-(sigma)*(squarenorm(x)-2*dot_product(x, z)+squarenorm(z)+b));
    }
